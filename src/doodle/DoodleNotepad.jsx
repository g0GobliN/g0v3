...
const DoodleNotepad = ({ isDarkMode, onClose }) => {
  ...
  const handleDragEnd = async () => {
    ...
    if (commentToUpdate) {
      try {
        ...
        console.error("Error updating document position:", e);
      }
    }
  ...
  }, [draggedComment, dragOffset, comments]);
  ...
  const saveComment = async () => {
    ...
    try {
      ...
      setComments(prevComments => [...prevComments, { id: docRef.id, ...newComment }]);
      cancelForm();
    } catch (e) {
  ...
  const cancelForm = () => {
    ...
    // The deleteComment function is now unused but is left here in case you need it later.
  ...
  if (showForm && formStep === 'draw') {
    return (
      <div className={`fixed inset-0 z-50 font-mono text-xs sm:text-sm p-2 sm:p-4 flex flex-col items-center transition-all duration-500 ${
        isDarkMode ? 'bg-black text-white' : 'bg-white text-black'
      }`}>
        <div className={`max-w-3xl w-full md:border md:border-gray-300 md:rounded-lg md:p-6 p-4 flex flex-col flex-1 transition-all duration-500 ${
          isDarkMode 
            ? 'md:border-gray-800' 
            : 'md:border-gray-300'
        }`}>
          ...
          <div className="mb-4 sm:mb-8">
            ...
            <div>
              <h1 className="text-sm sm:text-base md:text-lg font-light tracking-wide">sketch something amazing</h1>
        ...
        <div className={`max-w-3xl w-full md:border md:border-gray-300 md:rounded-lg md:p-6 p-4 flex flex-col flex-1 transition-all duration-500 ${
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="max-w-md w-full">
              ...
              {/* Preview */}
              {currentDoodle && (
                <div className={`mb-4 sm:mb-6 p-3 sm:p-4 rounded border ${isDarkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'}`}>
                  <img 
                    src={currentDoodle} 
                    alt="Your doodle" 
              ...
              {formStep === 'name' && (
                <div>
                  ...
                  <p className={`text-xs mb-4 sm:mb-6 ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
    ...
    <div className={`fixed inset-0 z-50 font-mono text-xs sm:text-sm p-2 sm:p-4 flex flex-col items-center transition-all duration-500 ${
      ...
      <div className={`max-w-3xl w-full md:border md:border-gray-300 md:rounded-lg md:p-6 p-4 flex flex-col flex-1 transition-all duration-500 ${
        ...
        <div className="mb-4 sm:mb-8">
          ...
          <div>
            <h1 className="text-sm sm:text-base md:text-lg font-light tracking-wide">interactive memory board</h1>
            <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>
        ...
        <div className="flex-1 relative comments-container min-h-64 sm:min-h-96">
          {comments.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className={`text-xs opacity-30 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          ...
          {comments.map((comment) => (
            <div
              key={comment.id}
              className={`absolute group cursor-move select-none ${draggedComment === comment.id ? 'z-50' : ''}`}
              style={{
            ...
            >
              <div className={`border-2 sm:border-4 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 ${
                isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'
              } ${draggedComment === comment.id ? 'shadow-2xl scale-105' : ''}`}>
                
                <div className={`h-20 sm:h-32 flex items-center justify-center border-b rounded-t ${
                ...
                {/* <button
                  ...
                  className={`absolute -top-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 text-xs opacity-0 group-hover:opacity-100 
                    transition-opacity flex items-center justify-center hover:bg-red-600 hover:text-white rounded-full ${
