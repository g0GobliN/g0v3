const BlogPost = ({ post, index, isDarkMode, expandedBlogPosts, toggleBlogPost }) => {
  const clickSound = new Audio('/assets/sounds/8bit.mp3');
  clickSound.volume = 0.3; // Adjust volume here (0.0 = mute, 1.0 = full volume)

  const handleToggle = (e) => {
    e.stopPropagation();
    clickSound.currentTime = 0; // Reset sound to start
    clickSound.play().catch(() => {}); // Prevent errors if not allowed yet
